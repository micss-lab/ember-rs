...
#[derive(Debug, Clone)]
pub(crate) struct FlatTokenStream {
    tokens: Vec<Token>,
}
...
impl FlatTokenStream {
    pub(crate) fn new(stream: TokenStream) -> FlatTokenStream {
        let mut tokens = vec![];
    ...
    pub(crate) fn next_span(&self, pos: usize) -> RuleResult<Span> {
    ...
    pub(crate) fn ident(&self, pos: usize) -> RuleResult<Ident> {
        match self.tokens.get(pos) {
    ...
    pub(crate) fn literal(&self, pos: usize) -> RuleResult<Literal> {
        match self.tokens.get(pos) {
...
#[derive(Debug, Clone)]
pub(crate) struct SpanPosition(pub(crate) Span, pub(crate) usize);
...
impl Parse for FlatTokenStream {
    type PositionRepr = SpanPosition;
    fn start(&self) -> usize {
        0
    }
    ...
    fn position_repr(&self, pos: usize) -> SpanPosition {
        let span = self
            .tokens
            .get(pos)
            .map_or_else(Span::call_site, |t| t.span());
        SpanPosition(span, pos)
    }
}
