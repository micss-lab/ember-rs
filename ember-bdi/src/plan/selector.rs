...
#[derive(Debug)]
pub struct PlanSelection<'a, 'e, A> {
    ...
    plans: Option<Iter<'a, Plan<A>>>,

    event: &'e TriggeringEvent,
}

impl<'a, 'e, A> PlanSelection<'a, 'e, A> {
    pub fn select_from_store(event: &'e TriggeringEvent, store: &'a PlanStore<A>) -> Self {
        Self {
            plans: store.plans.get(&event.into()).map(|p| p.iter()),
            event,
        }
    }
    ...
    fn relevant<'s>(&'s mut self) -> RelevantPlanSelection<'s, 'a, 'e, A> {
...
#[derive(Debug)]
struct RelevantPlanSelection<'s, 'a, 'e, A> {
    plans: Option<&'s mut Iter<'a, Plan<A>>>,

    event: &'e TriggeringEvent,
}

impl<'s, 'a, 'e, A> RelevantPlanSelection<'s, 'a, 'e, A> {
    fn applicable(self) -> ApplicablePlanSelection<'s, 'a, 'e, A> {
        ApplicablePlanSelection(self)
    }
}

impl<'a, 'e, A> RelevantPlanSelection<'_, 'a, 'e, A> {
    fn next_plan<'b>(&mut self) -> Option<(&'a Plan<A>, Bindings<'e>)>
    where
        'a: 'e,
    {
...
#[derive(Debug)]
struct ApplicablePlanSelection<'s, 'a, 'e, A>(RelevantPlanSelection<'s, 'a, 'e, A>);

impl<'a, 'e, A> ApplicablePlanSelection<'_, 'a, 'e, A> {
    fn next_plan<'b>(&mut self, knowledge: &'b BeliefBase) -> Option<(&'a Plan<A>, Bindings<'b>)>
    where
        'a: 'e,
        'e: 'b,
    {
...
mod tests {
    ...
    fn test_relevant_but_not_applicable() {
        ...
        let mut selection = PlanSelection::select_from_store(&event, &store);
        ...
        let mut selection2 = PlanSelection::select_from_store(&event, &store);
        assert!(
    ...
    fn test_backtracking_on_context_failure() {
        ...
        let (plan, _) = PlanSelection::select_from_store(&event, &store)
            .next_plan(&bb)
            .unwrap();
    ...
    fn test_context_negation_with_event_bindings() {
        ...
        assert!(
            PlanSelection::select_from_store(&event_north, &store)
                .next_plan(&bb)
                .is_none()
        );
        ...
        assert!(
            PlanSelection::select_from_store(&event_south, &store)
                .next_plan(&bb)
                .is_some()
        );
    }
