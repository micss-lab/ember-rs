...
use super::{Message, Performative};
use crate::acl::message::{Content, Receiver};
use crate::acl::sl::Content as SlContent;
use crate::agent::Aid;
...
impl MessageBuilder {
    fn build(self, performative: Performative) -> Result<Message> {
        ...
        let content = match self.language.as_ref().map(|l| l.as_slice()) {
            Some(b"fipa-sl0") => Content::Sl(SlContent::parse(content).map_err(|e| {
