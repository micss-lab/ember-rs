...
class HelloWorld: 
  ...
  public:
    virtual void action(framework::behaviour::Context<>& context) const override {
      ...
      Serial.println("My friend appears to print infinitely many messages, but will remove the agent after one.");
      Serial.println("You will notice my second copy will never run because of this.");
    }
};
